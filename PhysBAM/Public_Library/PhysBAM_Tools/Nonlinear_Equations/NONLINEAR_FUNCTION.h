...
template<class F> struct NONLINEAR_FUNCTION; // F must be a function type (e.g., T(T) or T(T1,T2))
template<class T,class F> struct PARAMETRIC_LINE; // F must be a two argument function type

template<class R,class T1>
struct NONLINEAR_FUNCTION<R(T1)>
{
...
struct NONLINEAR_FUNCTION<R(T1,T2)>
{
    virtual ~NONLINEAR_FUNCTION() {}
//#####################################################################
...
struct PARAMETRIC_LINE<T,R(T1,T2)>:public NONLINEAR_FUNCTION<R(T)>
{
...
struct PARAMETER_SPACE
{
    ...
    virtual void Op(T a,const PARAMETER_SPACE& x,T b,const PARAMETER_SPACE& y) {PHYSBAM_FUNCTION_IS_NOT_DEFINED();} // this=a*x+b*y
